...
func _anzeigen_ausschalten() -> void:
	$AufgabenPanel/UntenLinks.visible = false
...
func _erzeuge_aufgabe() -> void:
	...
	if operator == 0: #+
		...
		summand2 = randi() %6 +1
		summand3 = randi() %6 +1
		while (summand1 + summand3 > 7) or (summand2 + summand3 > 7) or (summand1 == summand2):
			summand1 = randi() %6 +1
			summand2 = randi() %6 +1
			summand3 = randi() %6 +1
		ergebnis1 = summand1 + summand3
		ergebnis2 = summand2 + summand3
	else: #-
		summand1 = randi() %6 +2
		summand2 = randi() %6 +2
		summand3 = randi() %6 +1
		while (summand1 - summand3 <= 0) or (summand2 - summand3 <= 0) or (summand1 == summand2):
			summand1 = randi() %6 +2
			summand2 = randi() %6 +2
			summand3 = randi() %6 +1
		ergebnis1 = summand1 - summand3
...
func _zahlen_eintragen(operator : int, zahl1 : int, zahl2 : int, zahl3 : int) -> void:
	...
	$AufgabenPanel/MitteLinks/ZahlMitteLinks.text = str(zahl1)
	$AufgabenPanel/UntenLinks/ZahlUntenLinks.text = str(zahl2)
	$AufgabenPanel/ObenRechts/ZahlOben.text = str(zahl3)


func _aktualisiere_Anzeige() -> void:
	$AufgabenPanel/UntenLinks.visible = true
