...
func _erzeuge_aufgabe() -> void:
	var dachzahl : int = randi() %2 +9
	$AufgabenPanel/Dach/Dachzahl.text = str(dachzahl)
	if dachzahl >= 10:
		$AufgabenPanel/Dach/Dachzahl.position.x -= 60
	
	#R1 erzeugen
	var zufall_handwert_r1 : int = randi() %9 +1
	while zufall_handwert_r1 >= dachzahl:
		zufall_handwert_r1 = randi() %9 +1
